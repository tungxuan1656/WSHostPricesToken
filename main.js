    ...
    return {
       ...
       */
        ...
        const indicators = await Promise.all(
          (chart.indicators || []).map((i) =>
            getIndicator(i.id, i.version, i.settings, i.type)
          )
        )
        ...
        async function updatePeriods(packet) {
          ...
          await Promise.all(
            Object.keys(newData).map(async (type) => {
              ...
              if (newData[type].ns && newData[type].ns.d) {
                ...
                if (
                  stratData.data &&
                  stratData.data.report &&
                  stratData.data.report.performance
                ) {
                  if (!strategies[std.name || type])
                    strategies[std.name || type] = { performance: {} }
                  strategies[std.name || type].performance =
                    stratData.data.report.performance
                  return
                }
              ...
              ;(newData[type].s || newData[type].st || []).forEach((p) => {
                ...
                if (newData[type].st) {
                  ...
                  p.v.forEach((val, i) => {
                    if (i === 0) return
                    if (
                      indicator.plots[`plot_${i - 1}`] &&
                      !period[indicator.plots[`plot_${i - 1}`]]
                    ) {
                      period[indicator.plots[`plot_${i - 1}`]] = val
                    } else period[`_plot_${i - 1}`] = val
                  })
                  periods[p.i][chart.indicators[parseInt(type, 10)].name || `st${type}`] =
                    period
                }
              })
            })
        ...
        chartCBs[chartSession] = async (packet) => {
          ...
          if (packet.type.endsWith('_error')) {
            handleError(
              `Error on '${chart.symbol}' (${chartSession}) chart: "${packet.type}":`,
              packet
            )
          }
        }
        ...
        indicators.forEach(async (indicator, i) => {
          ...
          Object.keys(indicator.inputs).forEach((inputID, inp) => {
            ...
            if (input.type === 'bool' && typeof input.value !== 'boolean')
              handleError(`Input '${input.name}' (${inp}) must be a boolean !`)
            if (input.type === 'integer' && typeof input.value !== 'number')
              handleError(`Input '${input.name}' (${inp}) must be a number !`)
            if (input.type === 'float' && typeof input.value !== 'number')
              handleError(`Input '${input.name}' (${inp}) must be a number !`)
            if (input.type === 'text' && typeof input.value !== 'string')
              handleError(`Input '${input.name}' (${inp}) must be a string !`)
            if (input.options && !input.options.includes(input.value)) {
              handleError(
                `Input '${input.name}' (${inp}) must be one of these values:`,
                input.options
              )
            }
